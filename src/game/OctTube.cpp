...
OctTube::OctTube(Context* ctx, std::shared_ptr<CachedFileLoader> loader) : Model(ctx), tube_mat_(ctx) {
  auto mesh = ctx->GetCachedFileLoader()->LoadModel("resources/octtube.obj");
  SetMesh(mesh);
  rot_ = 0.0f;
}
